...
@router.get("/formtwo", response_class=HTMLResponse)
def form_get(request: Request):
    key = os.getenv("unsplash_key")
    print(key)
    result = "Type a number"
    return templates.TemplateResponse('formtwo.html', context={'request': request, 'result': result})

@router.post("/formtwo", response_class=HTMLResponse)
def form_post2(request: Request, ticker2: str = Form(...), period: str = Form(...), interval: str = Form(...)):
    ...
    resp = {'1': 'Test response'}
    return templates.TemplateResponse('formtwo.html', context={'request': request, 'resp': resp})
