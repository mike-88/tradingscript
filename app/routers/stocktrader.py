...
@router.get("/stocktrader", response_class=HTMLResponse)
def form_get(request: Request):
    key = os.getenv("unsplash_key")
    print(key)
    result = "Type a number"
    return templates.TemplateResponse('stocktrader.html', context={'request': request, 'result': result})
...
def form_post2(request: Request, ticker2: str = Form(...), period: str = Form(...), interval: str = Form(...)):
    ...
    return templates.TemplateResponse('stocktrader.html', context={'request': request, 'ticker2': ticker2, 'trader_div': result_dict, 'pl_list':pl_list})